...
# stopped the old one. If the old one was still holding port 8002 (the
...
old_pid=$(ss -tlnp 2>/dev/null | awk '/:8002 /{print $0}' | grep -oP 'pid=\K[0-9]+' | head -1)
if [ -n "$old_pid" ]; then
    kill "$old_pid" 2>/dev/null
    for _ in $(seq 1 20); do
        ss -tln 2>/dev/null | grep -q ':8002 ' || break
        sleep 0.5
    done
    ss -tln 2>/dev/null | grep -q ':8002 ' && kill -9 "$old_pid" 2>/dev/null
fi
...
for _ in $(seq 1 30); do
    sleep 0.5
    curl -sf -o /dev/null http://127.0.0.1:8002/health && { echo "backend up, pid $new_pid"; exit 0; }
done
echo "FAILED: backend did not come up on port 8002 after restart (see $REPO/backend.log)" >&2
exit 1
