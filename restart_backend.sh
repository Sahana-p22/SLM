#!/bin/bash
set -x
REPO=/home/wgtech/slm-llama-approach2
...
export FQC_GGUF_MODEL_PATH="$REPO/models/Llama-3.2-3B-Instruct-GGUF/Llama-3.2-3B-Instruct-Q4_K_M.gguf"

# This script previously only ever launched a new uvicorn process and never
# stopped the old one. If the old one was still holding port 8006 (the
...
old_pid=$(ss -tlnp 2>/dev/null | awk '/:8006 /{print $0}' | grep -oP 'pid=\K[0-9]+' | head -1)
if [ -n "$old_pid" ]; then
    kill "$old_pid" 2>/dev/null
    for _ in $(seq 1 20); do
        ss -tln 2>/dev/null | grep -q ':8006 ' || break
        sleep 0.5
    done
    ss -tln 2>/dev/null | grep -q ':8006 ' && kill -9 "$old_pid" 2>/dev/null
fi

setsid nohup uvicorn chat.backend.main:app --host 127.0.0.1 --port 8006 > "$REPO/backend.log" 2>&1 < /dev/null &
new_pid=$!
disown

for _ in $(seq 1 30); do
    sleep 0.5
    curl -sf -o /dev/null http://127.0.0.1:8006/health && { echo "backend up, pid $new_pid"; exit 0; }
done
echo "FAILED: backend did not come up on port 8006 after restart (see $REPO/backend.log)" >&2
exit 1
